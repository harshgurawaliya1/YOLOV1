...
## the architecture of the model (kernel size, number of filters, stride, padding)
...
architecture_config = [
    ...
    (3, 512, 1, 1),
    "M",
    # List: tuples and then the last integer is number of repeats
...
class CNNBlock(nn.Module):
    ...
    def forward(self,x):
        ...
        batch_out = self.batchnorm(conv_out)
        activ_out = self.leakyrelu(batch_out)
        return activ_out
...
class Yolov1(nn.Module):
    ...
    def _create_fcs(self , split_size , num_boxes , num_classes ):
        ...
        return nn.Sequential(
            nn.Flatten(),
            nn.Linear(1024 * S * S, 4096 ), #original paper 4096
            nn.Dropout(0.0),
            nn.LeakyReLU(0.1),
